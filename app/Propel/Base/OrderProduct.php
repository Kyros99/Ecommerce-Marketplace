...
abstract class OrderProduct implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\App\\Propel\\Map\\OrderProductTableMap';
    ...
    /**
     * The value for the initial_price field.
     *
     * @var        string
     */
    ...
    /**
     * Initializes internal state of App\Propel\Base\OrderProduct object.
     */
    ...
    /**
     * Get the [initial_price] column value.
     *
     * @return string
     */
    ...
    public function setAmount($v)
    {
        ...
        if ($this->amount !== $v) {
            $this->amount = $v;
            $this->modifiedColumns[OrderProductTableMap::COL_AMOUNT] = true;
        }
    ...
    /**
     * Set the value of [initial_price] column.
     *
     * @param string $v New value
     * @return $this The current object (for fluent API support)
     */
    public function setInitialPrice($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->initial_price !== $v) {
            $this->initial_price = $v;
            $this->modifiedColumns[OrderProductTableMap::COL_INITIAL_PRICE] = true;
        }
    ...
    public function setTotalPrice($v)
    {
        ...
        if ($this->total_price !== $v) {
            $this->total_price = $v;
            $this->modifiedColumns[OrderProductTableMap::COL_TOTAL_PRICE] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : OrderProductTableMap::translateFieldName('OrderId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->order_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : OrderProductTableMap::translateFieldName('ProductId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->product_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : OrderProductTableMap::translateFieldName('Amount', TableMap::TYPE_PHPNAME, $indexType)];
            $this->amount = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : OrderProductTableMap::translateFieldName('InitialPrice', TableMap::TYPE_PHPNAME, $indexType)];
            $this->initial_price = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : OrderProductTableMap::translateFieldName('TotalPrice', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = OrderProductTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(OrderProductTableMap::COL_AMOUNT)) {
            $modifiedColumns[':p' . $index++]  = 'amount';
        }
        if ($this->isColumnModified(OrderProductTableMap::COL_INITIAL_PRICE)) {
            $modifiedColumns[':p' . $index++]  = 'initial_price';
        }
        if ($this->isColumnModified(OrderProductTableMap::COL_TOTAL_PRICE)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'amount':
                        ...
                        break;
                    case 'initial_price':
                        $stmt->bindValue($identifier, $this->initial_price, PDO::PARAM_STR);
    ...
    {
        if (isset($alreadyDumpedObjects['OrderProduct'][$this->hashCode()])) {
            return ['*RECURSION*'];
        }
        $alreadyDumpedObjects['OrderProduct'][$this->hashCode()] = true;
        $keys = OrderProductTableMap::getFieldNames($keyType);
        $result = [
            $keys[0] => $this->getOrderId(),
            $keys[1] => $this->getProductId(),
            $keys[2] => $this->getAmount(),
            $keys[3] => $this->getInitialPrice(),
    ...
    public function fromArray(array $arr, string $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) {
            $this->setInitialPrice($arr[$keys[3]]);
        }
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(OrderProductTableMap::COL_ORDER_ID)) {
            $criteria->add(OrderProductTableMap::COL_ORDER_ID, $this->order_id);
        }
        if ($this->isColumnModified(OrderProductTableMap::COL_PRODUCT_ID)) {
            $criteria->add(OrderProductTableMap::COL_PRODUCT_ID, $this->product_id);
        }
        if ($this->isColumnModified(OrderProductTableMap::COL_AMOUNT)) {
            $criteria->add(OrderProductTableMap::COL_AMOUNT, $this->amount);
        }
        if ($this->isColumnModified(OrderProductTableMap::COL_INITIAL_PRICE)) {
            $criteria->add(OrderProductTableMap::COL_INITIAL_PRICE, $this->initial_price);
        }
        if ($this->isColumnModified(OrderProductTableMap::COL_TOTAL_PRICE)) {
            $criteria->add(OrderProductTableMap::COL_TOTAL_PRICE, $this->total_price);
        }
    ...
    {
        $criteria = ChildOrderProductQuery::create();
        $criteria->add(OrderProductTableMap::COL_ORDER_ID, $this->order_id);
        $criteria->add(OrderProductTableMap::COL_PRODUCT_ID, $this->product_id);
    ...
    public function hashCode()
    {
        ...
        //relation fk__order_product_order to table orders
        ...
        //relation fk__order_product_product to table product
    ...
    {
        return (null === $this->getOrderId()) && (null === $this->getProductId());
    }
     ...
     * objects.
     *
     * @param object $copyObj An object of \App\Propel\OrderProduct (or compatible) type.
    ...
    public function copyInto(object $copyObj, bool $deepCopy = false, bool $makeNew = true): void
    {
        ...
        $copyObj->setInitialPrice($this->getInitialPrice());
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\Propel\OrderProduct Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    {
        if ($this->aOrders === null && ($this->order_id != 0)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aOrders->addOrderProducts($this);
             */
        }
    ...
    {
        if (null !== $this->aOrders) {
            $this->aOrders->removeOrderProduct($this);
        }
        if (null !== $this->aProduct) {
